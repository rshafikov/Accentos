...
- Инцидент: Образ виртуальной машины большого размера (30+гб) и не успевает загрузиться за время браузерной сессии.
    ...
    - Причина: Зависит от окружающий условий: малая скорость локальной сети, короткий тайм-аут на веб-сессию и т.п.; 
...
- Инцидент: при снятии снимка ВМ зависает "Запланирован" больше, чем на 30 минут, в то время как используемая для этого ВМ свободна к использованию.
    ...
    - Решение: Убедиться, что ВМ, с которой начали делать снимок готова к использованию (статус "Активка", "Отключена". Задача - "Нет"). ЕУдалить снимок со статусом "Запланирован". Запустиьть процесс создания снимка заново;
...
- Инцидент: При успешной загрузке образа в формате .ova, он отображается в Dashboard как формат .vmdk.
    ...
    - Причина: Так и должно быть. Система раскрывает .ova и достаёт оттуда данные для .vmdk; 
...
- Инцидент: При инициализации ВМ Windows 10 машина не отрабаывает скрипты из раздела "Конфигурация" или не меняет hostname на актуальный.
    ...
    - Причины: Отсутствует/не отработал пакет Cloudbase Solutions. Конкретная причина такого поведения может быть различная.
